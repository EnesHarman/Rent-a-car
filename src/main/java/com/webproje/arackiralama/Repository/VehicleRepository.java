...
public interface VehicleRepository extends JpaRepository<Vehicle, Integer>{
	...
	List<VehicleDto> listVehiclesByCompanyId(int companyId, Pageable pageable);
	
	@Query("Select new com.webproje.arackiralama.Entity.dto.vehicleDtos.VehicleDto "
			+ "(v.id,v.brand,v.model,v.deposit,v.kilometerLimit,v.price,v.ImgUrl,f.name,g.name,vs.name,c.id)"
			+ "From Vehicle v Inner Join v.fuelType f Inner Join v.gearType g Inner Join v.vehicleStatus vs  Inner Join v.company c  ")
	List<VehicleDto> listVehicles(Pageable pageable);
}
